...
bool Graphics::Init()
{
	...
	SDL_SetRenderDrawColor(mRenderer, 0x16, 0x25, 0x30, 0xFF);
...
void Graphics::Camera(Vector2 vec, bool center)
{
	if (center)
		mCamera->x = vec.x - mCamera->w * 0.5f;
	else
	{
		if (vec.x > mCamera->x + mCamera->w * 0.75f)
			mCamera->x = vec.x - mCamera->w * 0.75f;
		else if (vec.x < mCamera->x + mCamera->w * 0.25f)
			mCamera->x = vec.x - mCamera->w * 0.25f;
	}
