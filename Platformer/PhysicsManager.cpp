...
std::unordered_set<int> PhysicsManager::Index(PhysEntity* entity)
{
	...
	Vector2 min = Vector2(entity->Pos().x - radius, entity->Pos().y - radius);
	Vector2 max = Vector2(entity->Pos().x + radius, entity->Pos().y + radius);
...
void PhysicsManager::Update()
{
	...
	std::unordered_map<unsigned long, unsigned long> checkList;
	for (auto& entity : mEntity)
	{
		for (auto& other : CheckNearby(entity))
		{
			if (mLayerMasks[static_cast<unsigned int>(entity->CollisionLayer())].test(static_cast<unsigned int>(other->CollisionLayer())))
			{
				if (checkList.insert({ entity->Id(), other->Id() }).second)
				{
					checkList.insert({ other->Id(), entity->Id() });
